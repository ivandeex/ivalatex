...
\begin{proof}
	\begin{align*}
	P(X=1+n|X>1) &= P(X=1+n)	\\
	\implies \M(X|X>1) &= \M(1+X) = 1+\M(X)	\\
	\implies \M(X) &= \M(X|X=1) P(X=1) + \M(X|X>1) P(X>1) \\
...
\[ S_n=binomial(n,p_n) \text{ при } n\to\infty ,\;
p_n\to0 ,\; np_n\to\lambda \;\implies\;
P(S_n=k)\to \dfrac{e^{-\lambda}\lambda^k}{k!} \]
